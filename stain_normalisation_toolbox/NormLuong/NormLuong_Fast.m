...
if max(source_image_size)>=1e4
    ...
    tiles_color_var = cell2mat(cellfun( @(x) ...
        sum(var(reshape(double(x./255),size(x,1)*size(x,2),3),0,1)), tiles_rgb,'UniformOutput',false));

    % find the initial parameters from the most dense area
    nim2 = cell(numel(tiles_rgb),1);
    [~,indx] = max(1-tiles_pct_white(:) + tiles_color_var(:));
    rgb = tiles_rgb{indx};
    %T = tic;
    [nim2{indx}, init_params] = NormLuong_Fast(rgb, target_image, init_params);   
    %fprintf('Done with getting init param in %.2f\n',toc(T));
    non_white_indx = find(tiles_pct_white < 0.9);
    ...
    normalized_image = cell2mat(reshape(nim2,size(tiles_rgb)));
%     tic;
...
for n = 1:2
    ...
    %theta = theta_o + 3.34; clear theta_o
    ...
    elseif max(nrows,ncols) <= 1e4
        if isempty(init_params)
            ...
            tiles_pct_white = cell2mat(cellfun(@(x) ...
                ...
                tiles_rgb, 'UniformOutput',false));
            tiles_color_var = cell2mat(cellfun( @(x) ...
                sum(var(reshape(x,size(x,1)*size(x,2),3),0,1)), tiles_rgb,'UniformOutput',false));
            non_white_indx = find(tiles_pct_white < 0.6);
            %non_uniform_indx = find(tiles_color_var > prctile(tiles_color_var(:),90));
            %non_white_uniform_indx = find(tiles_pct_white < 0.6 & ...
            %    tiles_color_var > prctile(tiles_color_var(:),90));
            [~,sorted_id] = sort(tiles_color_var(non_white_indx), 'descend');
            % sort the color variance
            non_white_uniform_indx = non_white_indx(sorted_id);
            % only need to take the first 20 or 10
            non_white_uniform_indx = non_white_uniform_indx(1:min(10,length(non_white_uniform_indx)));
            % save values for converged iteration
            mu_hat_conv = {};
            kappa_hat_conv = {};
            mu_hat_nonconv = {};
            kappa_hat_nonconv = {};
            ...
            count_conv = 1;
            count_nonconv = 1;
            for tt = 1:length(non_white_uniform_indx)
               tile_theta = tiles_theta{non_white_uniform_indx(tt)};
               X_cart_tile = [cos(tile_theta(:))'; sin(tile_theta(:))']';
               [ mu_hat,~,kappa_hat, ~, ~, conv] =...
                        moVM(X_cart_tile,numClusters,opts_mixture);
               if conv
                   mu_hat_conv{count_conv} = mu_hat;
                   kappa_hat_conv{count_conv} = kappa_hat;
                   count_conv = count_conv + 1;
               else
                   mu_hat_nonconv{count_conv} = mu_hat;
                   kappa_hat_nonconv{count_conv} = kappa_hat;
                   count_nonconv = count_nonconv + 1;
               end
            end
            %toc
            if count_conv > 1
                mu_hat = median(cat(1,mu_hat_conv{:}),1);
                kappa_hat = median(cat(1,kappa_hat_conv{:}),1);
            else
                mu_hat = median(cat(1,mu_hat_nonconv{:}),1);
                kappa_hat = median(cat(1,kappa_hat_nonconv{:}),1);
            end
