function varargout = color_norm_gui(varargin)
% COLOR_NORM_GUI MATLAB code for color_norm_gui_jeff.fig
...
% Last Modified by GUIDE v2.5 22-Jul-2016 16:22:27

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @color_norm_gui_jeff_OpeningFcn, ...
                   'gui_OutputFcn',  @color_norm_gui_jeff_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function color_norm_gui_jeff_OpeningFcn(hObject, eventdata, handles, varargin)
...
handles.color_index = repmat([1:7],1,50);
handles.output_file = 'user_results_dan.txt';
...
function varargout = color_norm_gui_jeff_OutputFcn(hObject, eventdata, handles) 
...
if strcmp(eventdata.Key,'1') || strcmp(eventdata.Key,'numpad1')
    ...
    handles.axes8.Color=[0.941,0.941,0.941];
...
elseif strcmp(eventdata.Key,'3')|| strcmp(eventdata.Key,'numpad3')
    handles.axes8.Color=[0.69,0.69,0.973];
    handles.axes6.Color=[0.941,0.941,0.941];
    handles.axes5.Color=[0.941,0.941,0.941];
    handles.user_select = -1;
...
elseif strcmp(eventdata.Key,'return') && any(strcmp('user_select',fieldnames(handles)))
    ...
    finish_gui = display_output(handles);
    if finish_gui; close all; return; end
    ...
    axes(handles.axes3);imshow(im3);
    axes(handles.axes4);imshow(im4);
    handles = rmfield(handles, 'user_select');
else
    return;
end
...
function [msg_txt, table_rank] = rank_methods(handles)

norm_pairs = readtable(handles.output_file); 
...
function avg_inconst = prcm_incons(handles)
T = readtable(handles.output_file); 
% test the logic
count_inconst = 0;
for tt = 1:2
    for ss = 1:10
...
function finish_gui = display_output(handles)
if handles.count > height(handles.norm_pairs)
    % stop the GUI
    handles.norm_pairs.Results = handles.user_choices';
    writetable(handles.norm_pairs,handles.output_file);
    [msg_text, table_rank] = rank_methods(handles);
    avg_inconst = prcm_incons(handles);
    h = msgbox(sprintf('You have finished the test with inconsistency score of %.2f\n %s',...
        avg_inconst,msg_text),'Done');
    set(h, 'position', [100 440 500 200]); %makes box bigger
